...
- Git: Implementation, tests, E2E contract, product docs, and project memory were committed and pushed as `249e9f55b7e9f334c243560fb9227195cd31708d`; this pointer update is the final documentation-only auto-sync.
...
## 2026-07-20 - Page-load ahead drain and early slice activation

- Request: Replace the main image promptly after long-image preprocessing, process the loaded page once, and never update duplicate image sources.
- Changes: `ViewportImageProvider` now wires a guarded `window.load` handler, snapshots eligible images once, assigns canonical source owners, drains every unique snapshot entry through `aheadProcessingImageLimit`, and releases capacity on success, failure, cancellation, fallback, slice completion, rollback, disable, and page hide. Duplicate URLs with different render dimensions remain tracked as suppressed entries. Slice wrappers activate after all segment jobs register successfully, before backend results arrive; enhanced segment results progressively replace exact raw nodes and retain group-atomic rollback.
- Invariant/decision: One page-load snapshot is never rebuilt; later DOM images use normal viewport/prefetch promotion. A source owner may be reused by its same image after a source revision, but another image with the same canonical URL cannot enqueue or render a duplicate. Main-image replacement no longer waits for every segment inference result.
- Verification: Regression tests were added first and failed against the old behavior for pre-load ahead work, first-batch-only draining, duplicate URL dimensions, delayed slice activation, fallback ownership, and final-slice release. Final `scripts/verify.ps1` passed 59 backend tests, 208 extension tests, JavaScript checks, Ruff, and 73% backend coverage. Real Edge E2E passed with zero browser exceptions, 55/55 ready `768x32768` slices, two responsive wide tiles, settled worker/navigation/reload state, and settled queue/Referer rules.
- Git: Implementation, regression tests, product docs, and project memory are queued for the mandatory repository auto-sync after this verified change set.
- Remaining: A long-duration live-reader performance soak on the exact reported site is still the only release-level latency/memory evidence gap.

## 2026-07-21 - Resize-safe output sizing and text-preserving fast path

- Request: Fix manga pages whose dialogue and line shapes became malformed at 5% strength in non-automatic output modes, while reducing the cost of `Auto for image and screen`.
- Reproduction: Local logs/cache from the reported reader showed an `800x1583` source processed as a `136x270` neural input for a `544x1080` FHD result. The cached AI output visibly corrupted Vietnamese dialogue; larger manual output preserved more text but raised per-image inference from roughly 0.24 seconds to 1.5-1.9 seconds.
- Changes: Screen auto-orientation now follows source geometry; automatic sizing caps DPR at `1.5` and uses a `1.15` detail multiplier; targets at or below `1.5x` use Pillow Lanczos before existing enhancement/encoding; neural targets remain unchanged; extension cache identity is versioned as `pipeline:v2-resize-safe`.
- Invariant/decision: Never shrink a source to roughly one-quarter target dimensions merely to feed an `x4` model when the requested result is only a downscale or small upscale. Resize-only responses must report their real model/provider and remain cache-distinct from neural output.
- Verification: New source-orientation and high-DPI tests failed before implementation, then passed. A direct API run on the reproduced source returned `546x1080`, `model=lanczos`, `provider=Pillow`, `tileSize=null`, and zero GPU time; visual inspection preserved the original dialogue. Full verification passed 60 backend tests, 210 extension tests, JavaScript checks, Ruff, and 73% coverage. Real Edge E2E passed with zero browser exceptions, 55/55 tall slices, two responsive wide tiles, and settled queue, Referer-rule, worker, navigation, and reload state.
- Git: Implementation, tests, product docs, and project memory are ready for the mandatory repository auto-sync.

## 2026-07-21 - Collapsible Processing Monitor details

- Request: Add a control to collapse the Processing Monitor detail column so the jobs table can use the full Dashboard width.
- Changes: Added a semantic `Hide details` / `Show details` button beside Refresh, synchronized `aria-controls` and `aria-expanded`, a one-column collapsed layout, keyboard focus styling, and narrow-screen wrapping. The detail aside is hidden rather than rebuilt, so selected-job diagnostics and timeline DOM remain intact across collapse and restore.
- Verification: The new regression failed before implementation because the toggle initializer did not exist, then passed `5/5`. Fast verification passed `60` backend tests, `211` extension tests, and JavaScript syntax. The first Edge run was rejected by the pre-existing worker-reactivation timing assertion after the old image completed too early; an unchanged clean-profile rerun passed the real collapse/restore clicks with preserved detail content, zero browser exceptions, `55/55` tall slices, two responsive wide tiles, and settled queue, Referer, worker, navigation, and reload state.
- Git: Implementation, tests, E2E contract, product docs, and project memory were committed and pushed as `63fe9449b884e7aad71f9c85706530db51dec749`; this pointer update is the final documentation-only auto-sync.

## 2026-07-21 - Whole-page processing, strength-safe neural blending, and stale-runtime isolation

- Request: Process every eligible image after page load without duplicate work or reader lag, and make Enhancement Strength meaningful outside 4K without merging dialogue glyphs.
- Reproduction: The old live process on `127.0.0.1:8765` returned HTTP 200 while still running the pre-resize pipeline. A reproduced `800x1741 -> 884x1920` request entered the neural path despite a `1.10x` target; serialized slice requests also paid repeated original PNG re-encoding.
- Changes: Added health `pipelineVersion=3`, moved the active backend/native launcher/config/manifest to `127.0.0.1:8766`, and made the MV3 worker reject stale health responses. Schema-4 migration restores the one-shot whole-page ahead default for pre-v4 settings, lowers the safe default owner window to three, and keeps explicit current disables. Neural jobs now build a same-size Lanczos baseline and blend the model result by `enhanceLevel`; resize-safe targets never resolve an ONNX model. Browser-owned PNG originals are cached byte-for-byte and cache identities use `pipeline:v3-strength-blend`. Popup/Dashboard guidance now explains the slider semantics.
- Invariant/decision: `0%` is geometry-preserving baseline, low values are proportional AI contribution, and `100%` is full neural output. Every loaded page still snapshots once after `window.load`, drains all unique canonical sources through bounded owners, and processes later dynamic images only through viewport/prefetch promotion.
- Verification: Focused tests first failed for missing pipeline health, strength blending, resize-only routing, raw PNG reuse, stale launcher health, schema migration, and cache namespace; after implementation they passed. Full `scripts/verify.ps1` passed `64` backend tests, `214` extension tests, JavaScript syntax, Ruff, and `73%` coverage. The direct benchmark returned `model=lanczos`, `provider=Pillow`, zero GPU time, `882x1920`, `3.28 ms` input-cache time, and `354.21 ms` total latency. Edge fixture E2E passed with zero browser exceptions, `55/55` tall slices, two responsive wide tiles, settled queue/rules/worker/navigation/reload state, and 500-job Dashboard latency evidence; one earlier run hit the fixture's known worker-stop timing race and was not accepted.
- Runtime: The updated backend remains running on `8766`; the unrelated stale process on `8765` was not stopped or modified.

## 2026-07-22 - Effective strength-compute pipeline v4

- Request: Restore Enhancement Strength after the v3 blend made every setting look weak, keep `5%` genuinely fast, make non-4K presets respond to the slider, and allow `100%` to become visibly extreme.
- Reproduction: Code inspection confirmed v3 returned raw neural output at `100%` and skipped the former finishing stage. It also routed every target at or below `1.5x` through Lanczos regardless of strength. A reproduced `800x1741 -> 882x1920` 5% request spent almost all of its time in WebP encoding rather than inference.
- Changes: Pipeline v4 reserves `0-10%` for model-free Lanczos and WebP method `0`; `15-100%` can use neural inference for all presets. Neural input area grows monotonically with strength toward source detail under a `500,000` pixel cap. Neural output is resized to the exact Lanczos target, blended on a nonlinear curve, and receives progressively stronger denoise/sharpen/contrast finishing. Backend health, Native Messaging, extension compatibility, and output cache identity now require v4. Popup/Dashboard guidance describes the latency and distortion tradeoff.
- Invariant/decision: A 5% request must never resolve an ONNX model. Strength must change both compute and visible output, temporary neural work must remain bounded, and composition must never change the requested output geometry. Existing page-load ahead, dedupe, slicing, cancellation, and stale-operation contracts remain unchanged.
- Verification: Regression tests failed first for the old path, then passed for no-model 5%, non-4K neural routing, monotonic and hard-capped neural input dimensions, exact output sizing, aggressive 100% difference, exact slider payload persistence, v4 health rejection, and cache isolation. Full `scripts/verify.ps1` passed `69` backend tests, `216` extension tests, JavaScript syntax, Ruff, and `77%` coverage. Edge E2E passed with zero browser exceptions, `55/55` tall slices, two responsive wide tiles, offscreen ahead processing at `scrollY=0`, and fully settled queue/rule/lifecycle state.
- Benchmarks: Reproduced `800x1741 -> 882x1920` work completed at 5% in about `197 ms` with `lanczos`/`Pillow`, zero GPU, `0.383 ms` inference, and `66.524 ms` encode. At 100%, `anime_x4`/DirectML completed in about `2.60 s`, including `1.39 s` GPU, with measured `1.405x` sharpness gain and visually confirmed aggressive distortion.
- Runtime: Pipeline v4 remains healthy on `127.0.0.1:8766`. No machine shutdown was requested or performed.
- Git: Implementation, tests, benchmarks, product docs, and project memory are ready for the mandatory repository auto-sync.

## 2026-07-22 - Extension resolution matrix and live settings application

- Request: Test Extension controls at HD, Full HD, 2K, and custom Pixel/Auto/Screen modes with Strength 5%, 35%, and 100%; fix cases where resolution changes did not process images or Strength could not be adjusted.
- Reproduction: `resolveOutputLimits` showed Screen presets inheriting the hidden default Manual Pixel cap of `2048` px, so landscape 2K/4K could not reach their advertised dimensions. Content storage handling only reprocessed on `mode`/`enhanceLevel`, leaving existing images on the old resolution. Popup/Dashboard polling also wrote the saved Strength back into a focused range control.
- Changes: Screen mode now ignores Manual Pixel caps and emits exact preset dimensions. A content settings key set reprocesses existing operations for output sizing/quality, mode/Strength, performance, and text changes while excluding scheduling-only changes. Popup and Dashboard preserve focused controls during polling. Added `npm.cmd run test:e2e:edge-settings`, which exercises the real Popup, storage, content reprocessing, backend payloads, Blob dimensions, and cache identity.
- Verification: New regressions failed before implementation, then passed. `scripts/verify.ps1 -Fast` passed 69 backend tests, 220 extension tests, JavaScript checks, Ruff, and 77% coverage. Settings Edge E2E passed with zero browser exceptions: Auto 768x768, HD 5% request 1280x720/output 720x720, FHD 35% request 1920x1080/output 1080x1080, and 2K 100% request 2560x1440/output 1440x1440. The full Edge fixture also passed with zero browser exceptions, 55/55 tall slices, two wide tiles, Dashboard interaction, ahead processing, worker/navigation/reload lifecycle, and settled queues/rules.
- Invariant: A selected Screen preset is authoritative; changing a request-affecting setting must invalidate and rediscover existing content operations, while focused controls remain user-owned until blur.
- Git: Source/tests were auto-synchronized in `0c314c2`; the settings-only E2E command and final documentation are pending the next verified auto-sync.

## 2026-07-22 - Monitor persistence and lazy Dashboard list

- Request: Diagnose browser lag while the extension was active, identify resource bottlenecks, fix the monitor path, and make the Processing Monitor list collapsed until opened.
- Reproduction: A synthetic 500-job monitor with 3,500 timeline events took about 10.6 ms to clone and 4.7 ms to stringify per snapshot, producing about 2.7 MB per write. The Dashboard rebuilt the full table every poll, and a worker restart could accept a delayed enqueue for an operation already cancelled during recovery.
- Changes: Added summary-only and compact persistence modes to `ProcessingMonitorStore`; persistence keeps started work but caps idle `DETECTED` at 40, completed at 80, and errors at 80. The Dashboard now hides the job list by default, requests job payloads only when opened, uses single-flight/visibility-aware 3-second polling, and keeps the detail toggle inside the list panel. Background enqueue now waits for monitor recovery and rejects terminal operation identities.
- Invariant/decision: Diagnostic summary remains available without constructing job rows or transferring timelines; the full in-memory monitor remains available on demand. Persistence is bounded without dropping active started work, and a stale operation must never resurrect after worker restart.
...
## 2026-07-22 - Exact AI-result rejection and HentaiVNX banner safety

- Request: Fix the reported HentaiVNX false-positive replacement, add a precise ban action for the wrong AI result only, restore the original image after a ban, and avoid sending future identical blocked results through the heavy DOM/base64 path.
- Reproduction: HTTP inspection of the supplied chapter returned `200` and showed direct reader chrome at `/images/bn.png` beside `.page-chapter` images. The extension had only a source blacklist and no retained committed-original restore path; a blocked output could still be serialized to content before being discarded.
- Changes: Added cheap `/images/bn.png`, banner/branding, wrapper-promotion text, and cautious geometry filtering. Added schema-5 bounded `blockedResultRules` with exact query-preserving URL normalization. The Dashboard now separates `Ban wrong result` from `Block original`; background validates `tabId + imageId + operationId + enhancedImageUrl`, stores only the result URL, rejects matching cache/backend results before `sendComplete`, and sends a lightweight rejection for content cleanup. Renderer snapshots the first original DOM state and restores responsive attributes, picture sources, styles, classes, and owned Blob URLs after a committed ban. Processing Monitor accepts active-to-`SKIPPED` rejection transitions.
- Invariant/decision: An AI result URL is never a source blacklist key. A stale ban cannot mutate a newer operation, the original remains visible after rejection, and an exact blocked result cannot trigger another base64 render transaction. Generic chapter images using HentaiVNX's shared `alt` remain eligible.
- Verification: Focused red regressions were added first for Dashboard payload, background storage/forwarding, content rejection, renderer restore, schema migration, HentaiVNX promotion text, `/images/bn.png`, and pre-render blocked-result gating. `scripts/verify.ps1 -Fast` passed `69` backend tests, `235` extension tests, JavaScript syntax, and Ruff. Computer Use could not verify live adult-site DOM because policy blocked window-state inspection; no live Edge PASS is claimed for that site.
- Git: Source, tests, product docs, and project memory are ready for the mandatory repository auto-sync.

## 2026-07-22 - Result-ban Edge ordering regression fixed

- Issue: The new Edge result-ban assertion ran before the lookahead fixture. Because backend cache URLs can be reused across fixture images, the global blocked-result rule could skip lookahead and make the offscreen-render gate time out.
- Change: The E2E flow now proves offscreen lookahead completion before storing the exact AI-result ban, then verifies Dashboard storage, source-blacklist separation, original DOM restoration, and the existing geometry/lifecycle gates.
- Verification: `npm.cmd run test:e2e:edge-fixture` passed with `browserExceptions=0`, exact result URL ban, origin restoration, `55/55` tall slices, two responsive wide tiles, and settled queue/rule/worker/navigation/reload state. Full `scripts/verify.ps1` passed `69` backend tests, `235` extension tests, JavaScript checks, Ruff, and `77%` coverage.
- Decision: Keep the result-ban rule scoped to the exact enhanced URL and run it only after unrelated fixture work has completed; never use it as an original-source blacklist key.

## 2026-07-22 - Whole-page queued state and forward-reading priority

- Request: Explain and remove `Detected, not queued for preprocessing`, process every correct image, and prioritize from the current reading position toward the end of the chapter before work above the reader.
- Reproduction: `IMAGE_SEEN` created `DETECTED` records for all valid images, while `handlePageLoaded()` scheduled only the first `aheadProcessingImageLimit` entries and emitted no queued lifecycle event for the remaining backlog. Dynamic images also failed immediate admission because the lookup used only the active-operation map. Distance-only ordering did not distinguish below from above.
- Changes: Page-load and dynamic eligible images now enter a metadata-only ahead queue and emit one `PREPROCESSING_QUEUED` event before owning resources. Queue order is current viewport, below near-to-far, then above; scroll resorting uses captured document coordinates. Canonical duplicates emit `PREPROCESSING_SKIPPED` with `duplicate-source`. Sliced parents avoid duplicate queue events, benign bulk logs are suppressed, and disable/reprocess/pagehide terminalize unscheduled reported backlog entries.
...
## 2026-07-22 - Whole-page scroll responsiveness after Perfect 1

- Request: Keep full-page processing, immediate original-image visibility, current quality, Strength, resolution, and slicing while eliminating lag when the user scrolls quickly immediately after opening a chapter.
- Reproduction/analysis: the page-load pass repeated candidate geometry work, queue admission performed linear membership checks, every scroll could trigger priority work, large source fingerprints could synchronously decode base64, and multiple prepared results could reach visible DOM in the same frame. A real Edge recovery run also exposed that animation frames may stop in a background/headless tab, leaving a prepared result stuck in `rendering` unless a bounded fallback exists.
- Changes: reused discovery rectangles across candidate checks and initial ahead admission; added O(1) backlog keys; added a 160 ms scroll-active boundary that blocks only new offscreen ahead owners; kept visible `IntersectionObserver` promotion; converted large fingerprint bytes asynchronously; added a shared visible-first one-per-frame render queue with a 120 ms fallback; preserved originals and responsive attributes until commit; fixed first normal/fast-cached load identity; added timeout diagnostics to the Edge fixture.
- Invariant/decision: interaction backpressure must not lower enhancement quality, disable whole-page processing, hide originals, or relax operation identity. Visible work may progress during scroll, offscreen work resumes at idle, and every prepared render must settle even when animation frames are suspended.
- Verification: red-first focused regressions cover 500-image geometry reuse, O(1) admission, scroll-idle scheduling, no hot scroll reprioritization, asynchronous fingerprints, initial/cache-load identity, one commit per frame, visible-first scrolling, original visibility, and animation-frame fallback. `npm.cmd run test:extension` passed `247/247`. Full `scripts/verify.ps1` passed `69` backend tests, `247` extension tests, JavaScript checks, Ruff, and `77%` coverage. The final Edge fixture passed in `76.3 s` with `browserExceptions=0`, offscreen lookahead at `scrollY=0`, `55/55` tall slices, two responsive wide tiles, successful worker/navigation/reload lifecycle, and settled queue/rule/lock state.
- Limitation: no new long-duration soak was recorded on the exact public reader; the deterministic real Edge workload is the acceptance evidence for this checkpoint.
