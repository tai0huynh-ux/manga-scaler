...
- Dashboard image rows show the original URL from `IMAGE_SEEN` before preprocessing/AI completion, lazy-load remote sources at low priority, recover protected CDN bytes through one operation-checked background preview read, and cooldown failed preview URLs so monitor polling cannot create a request loop.
...
## Latest Extension settings delta (2026-07-22)

- Root cause: Screen preset requests were still clamped by hidden Manual Pixel `maxOutputWidth/maxOutputHeight` values, content scripts did not reprocess existing images when output sizing changed, and Popup/Dashboard polling could overwrite a focused Strength or mode control.
...
## Latest Dashboard origin-preview delta (2026-07-22)

- Root cause: Dashboard accepted only stable local/blob/data preview URLs, even though `IMAGE_SEEN` already carried the source URL for pending images; protected CDNs also needed the page Referer.
- Changes: Original rows now use the source URL immediately with lazy/low-priority loading. A failed remote preview makes one operation-checked `GET_ORIGINAL_PREVIEW` request through the existing browser reader and exact temporary Referer path, then renders transient data without persisting bytes. Failed URLs use a 30-second cooldown.
- Verification: Full `scripts/verify.ps1` passed `69` backend tests, `226` extension tests, JavaScript checks, Ruff, and `77%` backend coverage. Real Edge fixture proves a `seen` protected image becomes a loaded `data:image/png` origin preview, with `0` browser exceptions and settled queue/rules/worker/navigation state.

## Latest false-positive and result-ban delta (2026-07-22)

- Root cause: the HentaiVNX promotional `/images/bn.png` asset could be discovered when reader structure was incomplete, and there was no operation-safe way to reject one wrong enhanced URL without blacklisting the original source. Repeated blocked results also reached content as large base64 payloads before being discarded.
